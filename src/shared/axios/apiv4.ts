...
const apiVer4: AxiosInstance = axios.create({
  baseURL: process.env.NEXT_PUBLIC_API_URL_V4,
  timeout: 10000,
  headers: {
...
apiVer4.interceptors.request.use(
  (config) => {
...
apiVer4.interceptors.response.use(
  (response) => {
    return response;
  },
  async (error) => {
    ...
    if (isRefreshing) {
      return new Promise((resolve, reject) => {
        ...
        .then((token) => {
          return apiVer4(originalRequest);
        })
        .catch((err) => {
    ...
    if (res.code === SV_RES_STATUS_CODE.success) {
      ...
      return Promise.resolve(apiVer4(originalRequest));
    }
